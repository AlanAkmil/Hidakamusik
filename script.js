...
function setPlaying(yes){
  ...
  if(yes)startFpVisualizer();else stopFpVisualizer();
}

// ===== AMBIENT GLOW VISUALIZER (pseudo audio-reactive, gerakan halus + warna berubah) =====
let _fpVizRAF=null,_fpVizT=0;
function startFpVisualizer(){
  if(_fpVizRAF)return; // udah jalan
  const blobs=[document.getElementById('fpVizBlob1'),document.getElementById('fpVizBlob2'),document.getElementById('fpVizBlob3')];
  if(!blobs[0])return;
  function tick(){
    _fpVizT+=0.035;
    // Hue muter pelan & terus-menerus biar warnanya smooth berubah
    const baseHue=(_fpVizT*22)%360;
    blobs.forEach((b,i)=>{
      if(!b)return;
      // Kombinasi beberapa gelombang sinus beda fase/frekuensi biar geraknya organik, gak monoton
      const phase=i*2.1;
      const loud=Math.sin(_fpVizT*2.6+phase)*0.5+Math.sin(_fpVizT*4.8+phase*1.7)*0.35+Math.sin(_fpVizT*1.3+phase*0.6)*0.3;
      const intensity=Math.max(0.2,Math.min(1,(loud+1)/1.5));
      const scale=0.9+intensity*1.1;
      const riseY=-intensity*140;
      const hue=(baseHue+i*55)%360;
      b.style.transform='translateY('+riseY.toFixed(1)+'px) scale('+scale.toFixed(3)+')';
      b.style.opacity=(0.5+intensity*0.5).toFixed(2);
      b.style.background='radial-gradient(circle,hsl('+hue.toFixed(0)+',90%,58%) 0%,transparent 68%)';
    });
    _fpVizRAF=requestAnimationFrame(tick);
  }
  tick();
}
function stopFpVisualizer(){
  if(_fpVizRAF){cancelAnimationFrame(_fpVizRAF);_fpVizRAF=null;}
  ['fpVizBlob1','fpVizBlob2','fpVizBlob3'].forEach(id=>{
...
function openFullPlayer(){
  ...
  if(yt&&yt.getPlayerState&&yt.getPlayerState()===YT.PlayerState.PLAYING)startFpVisualizer();
}
