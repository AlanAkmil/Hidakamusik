...
function startFpVisualizer(){
  ...
  function tick(){
    ...
    const baseHue=(_fpVizT*14)%360;
    blobs.forEach((b,i)=>{
      if(!b)return;
      // Kombinasi beberapa gelombang sinus beda fase/frekuensi biar geraknya organik, gak monoton
      const phase=i*2.1;
      const loud=0.5+0.5*Math.sin(_fpVizT*1.7+phase)*0.6+0.4*Math.sin(_fpVizT*3.3+phase*1.5);
      const intensity=Math.max(0.15,Math.min(1,(loud+1)/2));
      const scale=0.75+intensity*0.6;
      const riseY=-intensity*70;
      const hue=(baseHue+i*55)%360;
      b.style.transform='translateY('+riseY+'px) scale('+scale.toFixed(3)+')';
      b.style.opacity=(0.35+intensity*0.4).toFixed(2);
      b.style.background='radial-gradient(circle,hsl('+hue.toFixed(0)+',85%,55%) 0%,transparent 70%)';
    });
